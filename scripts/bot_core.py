...
def index_source_text(message: dict[str, Any]) -> str:
    """Return text used for indexing.

    Rule:
    1. use current message text/caption first;
    2. if current media has no text, use replied message text/caption;
    3. if both are empty, do not index.
    """
    return own_message_text(message) or reply_message_text(message)


def message_media_type(message: dict[str, Any]) -> str:
    # Priority: file > audio > video > photo > text.
...
def index_message_with_text(conn, message: dict[str, Any]) -> bool:
    ...
    # message_indexer indexes message.text/caption only. For media that replies to a text
    # message, inject the replied text into a copied message for keyword extraction, while
    # keeping the current media message_id so the anchor link points to the media message.
