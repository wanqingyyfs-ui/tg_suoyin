...
`main` 分支采用“源码 + EXE”结构。GitHub Actions 在源码构建通过后，会把以下运行文件自动写回仓库：
...
正常情况下会显示用于构建该 EXE 的源码提交、Actions 运行编号和构建时间。

## 从 GitHub Actions 下载便携包

仓库根目录 EXE适合已经克隆源码的环境。需要完全独立的便携目录时：
...
本地构建不会自动覆盖仓库根目录 EXE。正式根目录 EXE由 GitHub Actions 在 `main` 分支构建成功后发布，避免把未经验证的本地构建误当成正式版本。
